...
```{r}
create_fytd_dd_plot(tib =fytd_code9_rate_changes, title_name = "**FYTD – Delayed Discharge Bed Days (Code 9 Delays)**")
...
```{r}
create_month_dd_plot(tib =mon_code9_rate_changes, title_name = "**{params$Report_month} {params$Report_year}– Delayed Discharge Bed Days (Code 9 Delays)**")
```

### Appendix 1 : HSCP Rankings for Financial Year to date `r fydate_text` (April 2021 to `r params$Report_month` `r params$Report_year`)

```{r}
gt(fytd_code9_app) %>% 
  tab_header(title = md(glue("**Appendix 1 : HSCP Rankings for Financial Year to date 20{fydate_text} (April 2021 to {params$Report_month} {params$Report_year})**")))
```
...
```{r}
create_fytd_dd_plot(tib =fytd_standard_rate_changes, title_name = "**FYTD – Delayed Discharge Bed Days (Standard Delays)**")
...
```{r}
create_month_dd_plot(tib =mon_standard_rate_changes, title_name = "**{params$Report_month} {params$Report_year}– Delayed Discharge Bed Days (Standard Delays)**")
...
temp_func <- function(df, ...){
  ...
  tmp_list <- list("a","b","c")
  list_length <- length(tmp_list)
  for (element in 1:list_length){
   out <- paste("This is element", element)
   print(out)
  }
}
```
