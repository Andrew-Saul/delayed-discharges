...
create_current_rateplot(tib=tib_FY_CA, CNeeds_string = "Code 9", fydate = "2122", mon = "December")
...
all_rates <- map(CA_names, ~get_rates(tib=tib_FY_CA, CA= .x, CNeeds_string = "Standard", fydate=2122, pop_objfile=LA_pop_18plus)) %>% 
  set_names(., CA_names)


map(all_rates, create_5yr_plot)

p <- create_5yr_plot(all_rates, index=27, CNeeds_string ="Standard")
p
ggplotly(p)
