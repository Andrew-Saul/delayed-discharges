...
CA_of_interest <- "Renfrewshire"

CA_of_interest_named_vec <- CA_of_interest %>% set_names()
# ---------------------------------------------------------------------------------------
...
code9_187475plus_obj <- create_FY_authority_187475plus(xl_file = code9_xl_name, sheet_name = "data", delay_reason = "All code 9 delay reasons")

standard_187475plus_ojb <- create_FY_authority_187475plus(xl_file = standard_xl_name, sheet_name = "data_1", delay_reason = "All standard")
# Create a common tibble containing both "standard"" and "code9" <delay reasons> 

# excludes FY 2016/17 and 2020/21

df_main <- create_main_tibble(code9_187475plus_obj, standard_187475plus_ojb) 
...
if(count(df_main %>% filter(is.na(obd)))>0){
  rm(df_main)
  stop("There are missing obd values in the dataset.  Please recheck")
...
# <recent_7yr_rates> function: 
# 1st list: tibble of last 7yr rates
# 2nd list: FYear column for prev 5yr data (excluding current year) - for plot and table functions
#           auto calculates min and max financial years for prev 5 years of data
#     
# 3rd list : FYear of all data.  Calcs current year, for use as row heading in table function

recent_7yr_rates <- get_prev_7yr_rates(df_main, delay_reasons_list, CA_of_interest) 
...
plot_table_func_list <- c("create_7yr_plot", "create_current_FY_tables")


for (count in seq_along(CA_of_interest)){ 
  arglist_i <- list(recent_7yr_rates, index=count, delay_reason=delay_reasons_list[[2]], CA_of_interest = CA_of_interest_named_vec[count])
...
code9_mon <- create_bedrate_list(tib=df_main, delay_reason = "All code 9 delay reasons", mon=params$Report_month, FYTD_flag=FALSE)

code9_fytd <- create_bedrate_list(tib=df_main, delay_reason = "All code 9 delay reasons", mon=params$Report_month, FYTD_flag=TRUE)

standard_mon <- create_bedrate_list(tib=df_main, delay_reason = "All Standard", mon=params$Report_month, FYTD_flag=FALSE)

standard_fytd <- create_bedrate_list(tib=df_main, delay_reason = "All Standard", mon=params$Report_month, FYTD_flag=TRUE)
```
...
fytd_standard_rate_changes <- get_summary_table(tib=df_main, mon, delay_reason = "All Standard", FYTD = TRUE) %>% 
  arrange(Rank) %>% 
  select(-c(pc_change_Avg_5yr))
...
mon_standard_rate_changes <- get_summary_table(tib=df_main, mon, delay_reason = "All Standard", FYTD = FALSE) %>% 
     arrange(Rank)                           
...
```{r}
for (count in seq_along(CA_of_interest)){ 
  arglist_i <- list(all_prev_rates, index=count, CNeeds_string="Standard", CA_of_interest = CA_of_interest[count])
