...
get_rates <- function(tib, CA, fydate, pop_objfile=LA_pop_18plus){
  ...
  #orders factor fin_yr
  tib_joined <- tib %>% 
    mutate(fin_yr = factor(fin_yr)) %>% 
    mutate(fin_yr = fct_reorder(fin_yr, dates, .desc = TRUE)) %>% 
    # mutate(Month_label = factor(months, labels = c("Apr", "May", "Jun", "Jul", "Aug", "Sep",
    #                                                "Oct", "Nov", "Dec", "Jan", "Feb",
    #                                                "Mar"))) %>% 
 ...
 left_join(prev5_rate, current_rate, by = "months") %>% 
   ...
   mutate(Month_label = case_when(months == 1 ~ "Apr", 
                                  months == 2 ~ "May", 
                                  months == 3 ~ "Jun", 
                                  months == 4 ~ "Jul", 
                                  months == 5 ~ "Aug", 
...
get_historic_counts <- function(tib, CA) {
  ...
  # create ordered factor fin_yr based on date 
  # with factor level =1 for most recent date
...
get_avg_rate <- function(tib){
  ...
  tib %>% 
    group_by(months) %>% 
    summarise(Avg_rate = 100000*sum(obd)/sum(Pop)) %>% 
    ungroup()  
}
...
# replaces recent NA pop values with previous year's values
# get_current_rate <- function(tib) {
#   tib %>% 
#     mutate(Pop = if_else(is.na(Pop), lag(Pop, n=sum(is.na(Pop))), Pop)) %>% 
#     filter(fin_yr == fydate_text) %>% 
#     filter(delay_reasons == CA) %>% 
#     drop_na(obd) %>% 
#     get_avg_rate() %>% 
#     rename(Current_rate = Avg_rate) 
#   }
